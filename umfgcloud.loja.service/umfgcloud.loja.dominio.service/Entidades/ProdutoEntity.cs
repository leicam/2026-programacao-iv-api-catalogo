...
public sealed class ProdutoEntity : AbstractEntity
{
    ...
    public void SetEAN(string ean) => EAN = ean ?? throw new ArgumentNullException(nameof(ean));

    public void SetValorCompra(decimal valorCompra)
    {
        if (valorCompra <= 0)
            throw new InvalidDataException($"Valor compra informado {valorCompra} invalido!");
    ...
    {
        if (valorVenda <= 0)
            throw new InvalidDataException($"Valor compra informado {valorVenda} invalido!");
